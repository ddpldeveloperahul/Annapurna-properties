...
class LeadAdmin(admin.ModelAdmin):
    list_display = ["id","display_id", "name", "mobile", "requirement_type", "property_type", "status", "assigned_to", "created_at"]
...
class LeadActivityAdmin(admin.ModelAdmin):
    list_display = ["id","lead", "activity_type", "description", "actor", "created_at"]
    list_filter = ["activity_type"]


@admin.register(Call)
class CallAdmin(admin.ModelAdmin):
    list_display = ["id","display_id", "from_number", "status", "direction", "duration_sec", "created_at"]
...
class CallEventAdmin(admin.ModelAdmin):
    list_display = ["id","call", "event_type", "provider_event_id", "created_at"]


@admin.register(CallTranscript)
class CallTranscriptAdmin(admin.ModelAdmin):
    list_display = ["id","call", "created_at"]


@admin.register(CallSummary)
class CallSummaryAdmin(admin.ModelAdmin):
    list_display = ["id","call", "requirement", "interest_level", "generated_by"]


@admin.register(AISession)
class AISessionAdmin(admin.ModelAdmin):
    list_display = ["id","call", "stage", "clarify_attempts", "created_at"]


@admin.register(Followup)
class FollowupAdmin(admin.ModelAdmin):
    list_display = ["id","display_id", "lead", "assigned_to", "followup_type", "status", "due_at"]
    list_filter = ["status", "followup_type"]


@admin.register(WhatsAppMessage)
class WhatsAppMessageAdmin(admin.ModelAdmin):
    list_display = ["id","display_id", "lead", "to_number", "status", "sent_at"]
    list_filter = ["status"]


@admin.register(AuditLog)
class AuditLogAdmin(admin.ModelAdmin):
    list_display = ["id","action", "object_type", "object_id", "actor_label", "created_at"]
    list_filter = ["action", "object_type"]


@admin.register(ApplicationInfo)
class ApplicationInfoAdmin(admin.ModelAdmin):
    list_display = ["id","name", "app_name"]
